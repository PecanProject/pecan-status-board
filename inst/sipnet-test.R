...
test_list <- read.csv("inst/test-data/sipnet-test-list.csv", comment.char = "#",
                      na.strings = "") %>%
  as_tibble() %>%
...
#final_log <- stages
#workflow_id <- final_log$workflow_id
...
write.csv(error_data, "data/sipnet-test/logs/sipnet-log-data.csv")

# Generate Data 

stages <- apply(stages,2,as.character)
write.csv(stages, "data/sipnet-test/sipnet-test_results.csv")
