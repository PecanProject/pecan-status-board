...
mod_maespa_ui <- function(id){
  ns <- NS(id)
  count <- maespa_report()
  maespa_count <- workflow_count(count)
...
#' maespa Server Functions
#'
#' @noRd 
mod_maespa_server <- function(id){
  moduleServer( id, function(input, output, session){
    ns <- session$ns
    result <- maespa_report()
    ...
    output$error_table <- DT::renderDT({
      error_data = read.csv("data/maespa-test/logs/maespa-log-data.csv")
