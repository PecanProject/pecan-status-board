...
mod_sipnet_ui <- function(id){
  ns <- NS(id)
  count <- sipnet_report()
  sipnet_count <- workflow_count(count)
...
#' sipnet Server Functions
#'
#' @noRd 
mod_sipnet_server <- function(id){
  moduleServer( id, function(input, output, session){
    ns <- session$ns
    result <- sipnet_report()
    ...
    output$error_table <- DT::renderDT({
      error_data = read.csv("data/sipnet-test/logs/sipnet-log-data.csv")
